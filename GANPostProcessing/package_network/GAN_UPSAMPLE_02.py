...
class DoubleConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, p, mid_channels=None):
        ...
        self.double_conv = nn.Sequential(
            nn.Conv2d(in_channels, mid_channels, kernel_size=p.KERNEL_SIZE, padding='same', bias=False),
            nn.BatchNorm2d(mid_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channels, out_channels, kernel_size=p.KERNEL_SIZE, padding='same', bias=False),
