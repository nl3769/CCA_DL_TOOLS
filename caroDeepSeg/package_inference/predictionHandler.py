...
class predictionClassFW():
    ...
    def buildMaps(self,
        ...
        for i in range(len(self.patches)):
            ...
            pos_ = data_["(x)"]
            self.map_prediction[:, pos_:pos_ + self.patch_width] = self.map_prediction[:,
                                                                   pos_:pos_ + self.patch_width] + prediction[i, :, :,
                                                                                                   0]
            self.map_overlay[:, pos_:pos_ + self.patch_width] = self.map_overlay[:,
                                                                pos_:pos_ + self.patch_width] + np.ones(
