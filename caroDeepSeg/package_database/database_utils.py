...
def adapt_segmentation(LI, MA, Odim):
    ...
    LI, MA = sp.signal_interpolation_1D(LI, grid_LI, grid_query), sp.signal_interpolation_1D(MA, grid_MA, grid_query)
    LI, MA = np.nan_to_num(LI, nan=0), np.nan_to_num(MA, nan=0)
...
def data_extraction_cubs(LI, MA, I, coordinates, pixel_width, pixel_height):
    ...
    # --- get roi
    LIc, MAc = np.nonzero(LI)[0], np.nonzero(MA)[0]
    # --- mask creation
    mask = np.zeros(I.shape, dtype=np.uint8)
    for i in range(LIc.shape[0]):
        id = int(round(LIc[i]))
        LI_, MA_ = int(LI[id]), int(MA[id])
        mask[LI_:MA_, id] = 255
    data = {'I': [],
            'M': [],
            'pname': []}
...
def data_extraction(LI1, LI2, MA1, MA2, I1, I2, OF, coordinates, pixel_width, pixel_height, pairs_name):
    ...
    # --- get roi
    LI1c, MA1c = np.nonzero(LI1)[0], np.nonzero(MA1)[0]
    LI2c, MA2c = np.nonzero(LI2)[0], np.nonzero(MA2)[0]
    rd.check_segmentation_dim(LI1c, LI2c, MA1c, MA2c, pairs_name)
    # --- mask creation
    mask1, mask2 = np.zeros(I1.shape, dtype=np.uint8), np.zeros(I2.shape, dtype=np.uint8)
    for i in range(LI1c.shape[0]):
        id = int(round(LI1c[i]))
        LI1_, MA1_ = int(LI1[id]), int(MA1[id])
        LI2_, MA2_ = int(LI2[id]), int(MA2[id])
