...
# ----------------------------------------------------------------------------------------------------------------------------------------------------
    def forward(self, fmap):
        ...
        q, k = map(lambda t: rearrange(t, 'b (h d) x y -> b h x y d', h=heads), (q, k)) # d is "free" dimension automically computed
        q = self.scale * q
        ...
        if self.position_only:
            ...
            sim = einsum('b h x y d, b h u v d -> b h x y u v', q, k) # dot product?
...
class Aggregate(nn.Module):
    ...
    def forward(self, attn, fmap):
        ...
        out = einsum('b h i j, b h j d -> b h i d', attn, v) # on multiplie et on somme les termes d'indice j (dernière dim de attn et 3ème de v)
