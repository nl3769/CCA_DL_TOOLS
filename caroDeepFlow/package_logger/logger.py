...
class loggerClass():
    ...
    def plot_loss(self):
        ...
        if len(self.loss_seg['validation']) != 0:
            ...
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            psave = os.path.join(self.p.PRES, "figure")
    ...
    def save_model_history(self):
        ...
        for key in keys:
            textfile = open(os.path.join(self.p.PRES, key + '.txt'), "w")
        ...
        if self.p.VALIDATION:
            ...
            if epoch > 0 and loss_val[-1] < np.min(loss_val[:-1]):
                ...
                for key in models.keys():
                    psave = os.path.join(self.p.PRES, "saved_models")
                    pufh.create_dir(psave)
    ...
    def plot_pred(self, I1, I2, M1_gt, M2_gt, M1_pred, M2_pred, OF_gt, OF_pred, epoch_id, fname):
        ...
        psave = os.path.join(self.p.PRES, "random_backup_prediction")
        pufh.create_dir(psave)
